...
public:
    ...
    {
        LOG_OBJ(LOG_TRACE, this) << L"rhs " << vbl::Hex((unsigned long)rhs);
        ...
        err = _wsplitpath_s(wszFileName, drive, _MAX_DRIVE, dir, _MAX_DIR, nullptr, 0, nullptr, 0);

        swprintf(wszFileName, MAX_PATH, L"%ls%ls", drive, dir);
    ...
    HRESULT __stdcall get_EXEName(BSTR* rhs)
    {
        ...
        LOG_OBJ(LOG_DEBUG, this) << L"rhs " << vbl::Hex((unsigned long)rhs);
        ...
        err = _wsplitpath_s(wszFileName, nullptr, 0, nullptr, 0, filename, _MAX_FNAME, nullptr, 0);
    ...
    static bool QueryVersionString(const std::vector<BYTE>& block, const wchar_t* name, std::wstring& out)
    {
        ...
        UINT cbTranslate = 0;
        if (VerQueryValueW(block.data(), L"\\VarFileInfo\\Translation", (LPVOID*)&lpTranslate, &cbTranslate) &&
            cbTranslate >= sizeof(*lpTranslate))
        {
            // use first translation
            wchar_t subkey[64];
            swprintf(subkey, _countof(subkey), L"\\StringFileInfo\\%04x%04x\\%s", lpTranslate[0].wLanguage, lpTranslate[0].wCodePage, name);
    ...
    static bool QueryFixedFileInfo(const std::vector<BYTE>& block, VS_FIXEDFILEINFO*& pInfo)
    {
        if (block.empty())
            return false;
        UINT cb = 0;
        if (!VerQueryValueW(block.data(), L"\\", (LPVOID*)&pInfo, &cb) || pInfo == nullptr)
            return false;
        return true;
    }
