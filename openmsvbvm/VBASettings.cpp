 ...
 */
EXPORT void __stdcall rtcSaveSetting(BSTR bstrAppName, BSTR bstrSection, BSTR bstrKey, BSTR bstrSetting)
{

    LOG(LOG_DEBUG) << L"strAppName '" << vbl::Bstr(bstrAppName) << L"', strSection '" << vbl::Bstr(bstrSection) << L"', strKey '"
                   << vbl::Bstr(bstrKey) << L"', strSetting '" << vbl::Bstr(bstrSetting) << L"'";
    ...
    swprintf(wcSubKey, MAX_PATH, VB_REGISTRY_PATH_BASE L"%ls\\%ls", bstrAppName, bstrSection);

    HKEY hkSettingKey;
    LSTATUS lResult;

    /* Create the settings key under wcSubkey in HKEY_CURRENT_USER */
    lResult = RegCreateKeyW(VB_REGISTRY_DEFAULT_HIVE, wcSubKey, &hkSettingKey);
    ...
    lResult = RegSetValueExW(hkSettingKey, bstrKey, NULL, REG_SZ, (const BYTE*)bstrSetting, strSafeGetLength(bstrSetting));
 ...
 */
EXPORT void __stdcall rtcDeleteSetting(BSTR bstrAppName, VARIANTARG vargSection, VARIANTARG vargKey)
{
    ...
    if (vargKey.vt == VT_ERROR)
    {
        if (vargSection.vt == VT_ERROR)
        {
            ...
            LOG(LOG_DEBUG) << L"Erase all app settings '" << vbl::Bstr(bstrAppName) << L"'";
            ...
            swprintf(wcSubKey, MAX_PATH, VB_REGISTRY_PATH_BASE L"%ls", bstrAppName);
        ...
        else
        {
            ...
            LOG(LOG_DEBUG) << L"Erase entire section settings '" << vbl::Bstr(bstrAppName) << L"', '" << vbl::Bstr(bstrSection) << L"'";
    ...
    else
    {
        ...
        LOG(LOG_DEBUG) << L"Erase specific key setting '" << vbl::Bstr(bstrAppName) << L"', '" << vbl::Bstr(bstrSection)
                       << L"', '" << vbl::Bstr(bstrKey) << L"'";
        ...
        /* Open that key */
        lResult = RegOpenKeyExW(VB_REGISTRY_DEFAULT_HIVE, wcSubKey, 0, KEY_WRITE, &hkKey);
        ...
        lResult = RegDeleteValueW(hkKey, bstrKey);
 ...
 */
EXPORT BSTR __stdcall rtcGetSetting(BSTR bstrAppName, BSTR bstrSection, BSTR bstrKey, VARIANTARG vargDefaultValue)
{
    ...
    /* Erase a specific Key */
    LOG(LOG_DEBUG) << L"Erase specific key setting '" << vbl::Bstr(bstrAppName) << L"', '" << vbl::Bstr(bstrSection) << L"', '"
                   << vbl::Bstr(bstrKey) << L"'";
    ...
    /* Open that key */
    lResult = RegOpenKeyExW(VB_REGISTRY_DEFAULT_HIVE, wcSubKey, 0, KEY_READ, &hkKey);
    BSTR bstrRet;
    ...
    lResult = RegQueryValueExW(hkKey, bstrKey, NULL, &dwType, NULL, &dwDataSize);
    ...
    bstrRet = SysAllocStringLen(NULL, (dwDataSize / 2) - 1);
    ...
    /* Get the specified value */
    lResult = RegQueryValueExW(hkKey, bstrKey, NULL, &dwType, reinterpret_cast<LPBYTE>(bstrRet), &dwDataSize);
