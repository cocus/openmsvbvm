 ...
 */
EXPORT void __stdcall __vbaAryLock(SAFEARRAY** ppsaDest, SAFEARRAY* psaSource)
{

    LOG(LOG_DEBUG) << L"Locking array " << vbl::Hex((unsigned long)psaSource);
 ...
 */
EXPORT void __stdcall __vbaAryUnlock(SAFEARRAY** ppsaSafeArray)
{

    LOG(LOG_DEBUG) << L"Unlocking array " << vbl::Hex((unsigned long)ppsaSafeArray);
 ...
 */
EXPORT LONG __stdcall __vbaLbound(signed int iDimension, SAFEARRAY* psaSafeArray)
{
    if (iDimension < 1)
    {
        vbaRaiseException(VBA_EXCEPTION_SUBSCRIPT_OUT_OF_RANGE);
 ...
 */
EXPORT LONG __stdcall __vbaUbound(signed int iDimension, SAFEARRAY* psaSafeArray)
{
    if (iDimension < 1)
    {
        vbaRaiseException(VBA_EXCEPTION_SUBSCRIPT_OUT_OF_RANGE);
    ...
    return psabDimensionBounds->lLbound + psabDimensionBounds->cElements;
} /* __vbaUbound */
 ...
 */
EXPORT void __stdcall __vbaAryConstruct2(SAFEARRAY* psaDest, SAFEARRAY* psaSource, void* lpVoidArg)
{
    /* Copy the source array psaSource to the destination array psaDest, including the bounds */
    memcpy(psaDest, psaSource, sizeof(SAFEARRAY) + (psaSource->cDims > 0u ? (sizeof(SAFEARRAYBOUND) * (psaSource->cDims - 1)) : 0));
    ...
    if (lpVoidArg)
    {
        ...
        if (psaDest->fFeatures & FADF_RECORD)
        {
            ...
            SafeArraySetRecordInfo(psaDest, reinterpret_cast<IRecordInfo*>(lpVoidArg));
        }
        ...
        else if (psaDest->fFeatures & FADF_HAVEIID)
        {
            ...
            psaDest->rgsabound[0].lLbound = 0;
            SafeArraySetIID(psaDest, (const GUID&)lpVoidArg);
        }
 ...
 *									destroyed.
 */
EXPORT void __stdcall __vbaErase(int* pData, SAFEARRAY** ppSafeArray)
{

    LOG(LOG_DEBUG) << L"pData " << vbl::Hex((unsigned long)pData) << L", ppsaSafeArray " << vbl::Hex((unsigned long)ppSafeArray);
 ...
 */
EXPORT void __stdcall __vbaAryDestruct(int* pData, SAFEARRAY** ppSafeArray)
{
    ...
    if (((*ppSafeArray)->fFeatures & FADF_EMBEDDED) || ((*ppSafeArray)->fFeatures & FADF_AUTO))
    {
 ...
 */
EXPORT void __cdecl __vbaRedimPreserve(int iUnk1, int iUnk2, SAFEARRAY** ppsaSafeArray, int iUnk3, unsigned int uiDimensions, ...)
{

    LOG(LOG_DEBUG) << L"ppsaSafeArray " << vbl::Hex((unsigned long)ppsaSafeArray) << L", uiDimensions "
                   << vbl::Hex((unsigned long)uiDimensions);
    ...
    if (uiDimensions == 1)
    {
        ...
        LOG(LOG_DEBUG) << L"iNewElementCount = " << iNewElementCount << L", PreviousElementCount "
                       << (*ppsaSafeArray)->rgsabound[0].cElements;

        SAFEARRAYBOUND sabNewBound;
        memcpy(&sabNewBound, &(*ppsaSafeArray)->rgsabound[0], sizeof(sabNewBound));
        sabNewBound.cElements = iNewElementCount;

        HRESULT hr = SafeArrayRedim(*ppsaSafeArray, &sabNewBound);
    }
    else
    {
 ...
 */
EXPORT void __cdecl __vbaRedim(USHORT uFeatures, ULONG ulElementsSize, SAFEARRAY** ppsaOut, void* lpArgument, USHORT uiDimensions, ...)
{
    if (!ppsaOut)
    {
        // Wtf?
    ...
    if (uFeatures & (FADF_RECORD | FADF_HAVEIID | FADF_HAVEVARTYPE))
    {
        ...
        hr = SafeArrayAllocDescriptorEx(vtArrayVarType, uiDimensions, ppsaOut);
        ...
        if (uFeatures & FADF_HAVEIID)
        {
            ...
            hr = SafeArraySetIID(*ppsaOut, reinterpret_cast<const GUID&>(lpArgument));
        ...
        else if (uFeatures & FADF_RECORD)
        {
            ...
            hr = SafeArraySetRecordInfo(*ppsaOut, reinterpret_cast<IRecordInfo*>(lpArgument));
    ...
    else
    {
        ...
        hr = SafeArrayAllocDescriptor(uiDimensions, ppsaOut);
