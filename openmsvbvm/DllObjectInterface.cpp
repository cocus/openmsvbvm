 ...
 */
EXPORT HRESULT __stdcall __vbaNew2(vba_new_data_arg_t* pvbNewData, IUnknown** ppv)
{

    HRESULT hr = CLASS_E_CLASSNOTAVAILABLE;

    LOG(LOG_DEBUG) << L"pvbNewData " << vbl::Hex((unsigned long)pvbNewData) << L", reval " << vbl::Hex((unsigned long)ppv);
    ...
    {
        // LOG(LOG_DEBUG) << L"CoClass: " << vbl::Guid(*pvbNewData->lpguidCoClass);
    }

    if (pvbNewData->lpguidInterface)
    {
        LOG(LOG_DEBUG) << L"Interface: " << vbl::Guid(*pvbNewData->lpguidInterface);
    }
    ...
    if (pvbNewData->flags & 2)
    {
        ...
        {
            LOG(LOG_DEBUG) << L"Got VBGlobal CoClass, using internal class";
...
_Check_return_ STDAPI DllGetClassObject(_In_ REFCLSID rclsid, _In_ REFIID riid, _Outptr_ LPVOID FAR* ppv)
{
    ...
    *ppv = nullptr;
    ...
    {
        LOG(LOG_DEBUG) << L"Requested PropertyBag class object, but not implemented";
        // TODO: Implement PropertyBag
        return E_NOTIMPL;
    }
    else
    {
        LOG(LOG_DEBUG) << L"Requested CLSID not available: " << vbl::Guid(rclsid);
    }
...
STDAPI VBDllGetClassObject(
    _In_ HMODULE* phModule, int lReserved, _In_ struct VBHeader* pVBHeader, _In_ REFCLSID rclsid, _In_ REFIID riid, _Outptr_ LPVOID FAR* ppv)
{
    return CLASS_E_CLASSNOTAVAILABLE;
}

STDAPI VBDllCanUnloadNow(struct VBHeader* pNewVBHeader)
{
    return S_FALSE;
}
