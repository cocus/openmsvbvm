...
static std::map<vbaExceptions, const wchar_t*> ExceptionDescriptionMapping = {
    ...
    {VBA_EXCEPTION_PROPERTY_OR_METHOD_CALL_INCLUDING_REFERENCE_TO_PRIVATE_OBJECT,
     L"A property or method call cannot include a reference to a private object, either as an argument or as a return value"},
    ...
    {VBA_EXCEPTION_SPECIFIED_FILE_OUT_OF_DATE_PROGRAM_REQUIRES_LATER_VERSION,
     L"The specified file is out of date. This program requires a later version"},
    ...
    {VBA_EXCEPTION_COMPONENT_CANT_CREATE_OBJECT_OR_RETURN_REFERENCE_TO_THIS_OBJECT,
     L"Component can't create object or return reference to this object"},
    {VBA_EXCEPTION_CLASS_DOESNT_SUPPORT_AUTOMATION, L"Class doesn't support Automation"},
    {VBA_EXCEPTION_FILE_NAME_OR_CLASS_NAME_NOT_FOUND_DURING_AUTOMATION_OPERATION,
     L"File name or class name not found during Automation operation"},
    ...
    {VBA_EXCEPTION_CONNECTION_TO_TYPE_LIBRARY_OR_OBJECT_LIBRARY_FOR_REMOTE_PROCESS_HAS_BEEN_LOST,
     L"Connection to type library or object library for remote process has been lost"},
    ...
    {VBA_EXCEPTION_THIS_KEY_IS_ALREADY_ASSOCIATED_WITH_AN_ELEMENT_OF_THIS_COLLECTION,
     L"This key is already associated with an element of this collection"},
    ...
    {VBA_EXCEPTION_PROBLEM_GETTING_PRINTER_INFORMATION_FROM_SYSTEM,
     L"Problem getting printer information from the system. Make sure the printer is set up correctly"},
 ...
 */
CEXTERN vbaExceptions __stdcall vbaErrorFromHRESULT(HRESULT hr)
{
    // TODO: use a map
    switch (hr)
    {
    ...
    case STG_E_INVALIDFUNCTION:
        return VBA_EXCEPTION_INVALID_PROCEDURE_CALL; // Unable to perform requested operation
    case STG_E_FILENOTFOUND:
        return VBA_EXCEPTION_FILE_NAME_OR_CLASS_NAME_NOT_FOUND_DURING_AUTOMATION_OPERATION; // %1 could not be found.
    case STG_E_PATHNOTFOUND:
        return VBA_EXCEPTION_PATH_NOT_FOUND; // The path %1 could not be found.
    case STG_E_TOOMANYOPENFILES:
        return VBA_EXCEPTION_TOO_MANY_FILES; // There are insufficient resources to open another file.
    case STG_E_ACCESSDENIED:
        return VBA_EXCEPTION_PERMISSION_DENIED; // Access denied.
    case STG_E_INSUFFICIENTMEMORY:
        return VBA_EXCEPTION_OUT_OF_MEMORY; // There is insufficient memory available to complete operation.
    case STG_E_DISKISWRITEPROTECTED:
        return VBA_EXCEPTION_PERMISSION_DENIED; // Disk is write-protected. TODO: Check if it's the correct return value
    case STG_E_SEEKERROR:
        return VBA_EXCEPTION_DEVICE_IO_ERROR; // An error occurred during a seek operation. TODO: Check if it's the correct return value
    case STG_E_WRITEFAULT:
        return VBA_EXCEPTION_DEVICE_IO_ERROR; // A disk error occurred during a write operation. TODO: Check if it's the correct return value
    case STG_E_READFAULT:
        return VBA_EXCEPTION_DEVICE_IO_ERROR; // A disk error occurred during a read operation. TODO: Check if it's the correct return value
    case STG_E_FILEALREADYEXISTS:
        return VBA_EXCEPTION_FILE_ALREADY_EXISTS; // %1 already exists.
    case STG_E_MEDIUMFULL:
        return VBA_EXCEPTION_DISK_FULL; // There is insufficient disk space to complete operation.
    case STG_E_UNKNOWN:
        return VBA_EXCEPTION_INTERNAL_ERROR; // An unexpected error occurred.
    ...
    case E_OUTOFMEMORY:
        return VBA_EXCEPTION_OUT_OF_MEMORY;
    case E_INVALIDARG:
        return VBA_EXCEPTION_AUTOMATION_ERROR;
    case E_NOTIMPL:
        return VBA_EXCEPTION_OBJECT_DOESNT_SUPPORT_THIS_ACTION;
    case DISP_E_BADVARTYPE:
        return VBA_EXCEPTION_AUTOMATION_ERROR;
    case DISP_E_ARRAYISLOCKED:
        return VBA_EXCEPTION_ARRAY_FIXED_OR_TEMPORARILY_LOCKED;
    default:
        return VBA_EXCEPTION_INTERNAL_ERROR; // TODO: Is this right?
    }
} /* vbaErrorFromHRESULT */
 ...
 */
CEXTERN void vbaRaiseException(vbaExceptions exceptionCode, EXCEPINFO const* pExcepInfo)
{

    LOG(LOG_DEBUG) << L"exceptionCode = " << (int)exceptionCode << L", raising SEH Exception to die()";
    ...
    RaiseException(VB_MAGIC_EXCEPTION_CODE, 0, VB_EXCEPTION_NUMBER_OF_ARGUMENTS, VBExceptionArguments);
} /* vbaRaiseException */
...
EXPORT int __stdcall __vbaExitProc(void)
{
    ...
    std::vector<vbaOnErrorRegistredHandler>::const_iterator cit;
    {
        ...
        for (cit = g_tRegistredOnErrorHandlers.cbegin(); cit != g_tRegistredOnErrorHandlers.cend(); ++cit)
        {
            ...
            {
                LOG(LOG_DEBUG) << L"Found current thread id in the registred on error handlers vector, removing it!";
...
EXPORT int __stdcall __vbaOnError(int iUnk1)
{
    ...
    LOG(LOG_DEBUG) << L"iUnk1 " << vbl::Hex((unsigned long)iUnk1);
    ...
    std::vector<vbaOnErrorRegistredHandler>::const_iterator cit;
    {
        ...
        for (cit = g_tRegistredOnErrorHandlers.cbegin(); cit != g_tRegistredOnErrorHandlers.cend(); ++cit)
        {
            if (cit->hThreadId == ::GetCurrentThreadId())
            {
                LOG(LOG_DEBUG) << L"Found current thread id in the registred on error handlers vector!";
                return 0;
            }
        }
 ...
 */
static bool IsVbaExceptionCode(struct _EXCEPTION_RECORD* ExceptionRecord)
{
 ...
 */
static void ShowUnhandledVBException(struct _EXCEPTION_RECORD* ExceptionRecord, void* EstablisherFrame, struct _CONTEXT* ContextRecord)
{
    wchar_t buff[300] = {0};
    ...
    struct _EXCEPTION_RECORD* ExceptionRecord, void* EstablisherFrame, struct _CONTEXT* ContextRecord, void* DispatcherContext)
{
    ...
    LOG(LOG_DEBUG) << L"ExceptionRecord " << vbl::Hex((unsigned long)ExceptionRecord) << L", EstablisherFrame "
                   << vbl::Hex((unsigned long)EstablisherFrame) << L", ContextRecord " << vbl::Hex((unsigned long)ContextRecord)
                   << L", DispatcherContext " << vbl::Hex((unsigned long)DispatcherContext);

    LOG(LOG_DEBUG) << L"ExceptionRecord->ExceptionFlags " << vbl::Hex((unsigned long)ExceptionRecord->ExceptionFlags);

    if (IsVbaExceptionCode(ExceptionRecord) == false)
    {
        LOG(LOG_DEBUG) << L"Not a VB Exception, returning ExceptionContinueSearch";
        return ExceptionContinueSearch;
    }

    if (!EstablisherFrame)
    {
        LOG(LOG_DEBUG) << L"EstablisherFrame == NULL, returning ExceptionContinueSearch";
        // This is not a VB exception if EstablisherFrame is null
        return ExceptionContinueSearch;
    }

    LOG(LOG_DEBUG) << L"VB Exception detected, g_iLastErrorCode = " << vbl::Hex((unsigned long)g_iLastErrorCode, 4);
    ...
    std::vector<vbaOnErrorRegistredHandler>::const_iterator cit;
    {
        ...
        for (cit = g_tRegistredOnErrorHandlers.cbegin(); cit != g_tRegistredOnErrorHandlers.cend(); ++cit)
        {
            if (cit->hThreadId == ::GetCurrentThreadId())
            {
                ...
                LOG(LOG_DEBUG) << L"ESP " << vbl::Hex((unsigned long)localStorage->ESP) << L", lpProcedureInfo "
                               << vbl::Hex((unsigned long)localStorage->lpProcedureInfo);
                ...
                {
                    LOG(LOG_DEBUG) << L"lpProcedureInfo == NULL, returning ExceptionContinueSearch";
                    // Weird!
                    return ExceptionContinueSearch;
                }
                ...
                {
                    LOG(LOG_DEBUG) << L"lpProcedureInfo IsBadReadPtr!, returning ExceptionContinueSearch";
                    return ExceptionContinueSearch;
                }
                ...
                {
                    LOG(LOG_DEBUG) << L"Calling cleanup function before resuming, function at "
                                   << vbl::Hex((unsigned long)localStorage->lpProcedureInfo->lpCleanupFunction);
                    ...
                    {
                        LOG(LOG_DEBUG) << L"localStorage->lpProcedureInfo->lpCleanupFunction IsBadCodePtr!, returning ExceptionContinueSearch";
                ...
                if ((localStorage->lpProcedureInfo->wFlags & PROCEDURE_INFO_FLAG_ON_ERROR_SET))
                {
                    ...
                    if (localStorage->lpProcedureInfo->lpOnErrorInfo)
                    {
                        ...
                        LOG(LOG_DEBUG) << L"On Error GoTo detected, setting EBP to " << vbl::Hex((unsigned long)ContextRecord->Ebp)
                                       << L", ESP to " << vbl::Hex((unsigned long)ContextRecord->Esp) << L", EIP to "
                                       << vbl::Hex((unsigned long)ContextRecord->Eip) << L" and returning";
                    }
                    ...
                    else
                    {
                        ...
                        LOG(LOG_DEBUG) << L"On Error Resume Next detected, block index " << (err_level + 1)
                                       << L", setting EBP to " << vbl::Hex((unsigned long)ContextRecord->Ebp) << L", ESP to "
                                       << vbl::Hex((unsigned long)ContextRecord->Esp) << L", EIP to "
                                       << vbl::Hex((unsigned long)ContextRecord->Eip) << L" and returning";
                    }
                    ...
                    {
                        LOG(LOG_DEBUG) << L"ContextRecord->Eip IsBadCodePtr!, returning ExceptionContinueSearch";
                        return ExceptionContinueSearch; // Weird!
                    }

                    LOG(LOG_DEBUG) << L"ContextRecord->Eip IsBadCodePtr is not bad code, continuing";
                    return ExceptionContinueExecution;
                }
                else
                {
                    LOG(LOG_DEBUG) << L"not sure about this case";
                    return ExceptionContinueSearch; // Weird!
                }
            }
        }
    ...
    LOG(LOG_DEBUG) << L"Unhandled VB exception!";
...
EXPORT void __fastcall __vbaFPException(void)
{
    ...
    LOG(LOG_DEBUG) << L"statusfp = " << fp_status;
