...
static BOOL CALLBACK InputBoxDlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
    ...
    case WM_COMMAND:
    {
        if ((LOWORD(wParam) == IDC_BUTTON_OK) || (LOWORD(wParam) == IDC_BUTTON_CANCEL) || (LOWORD(wParam) == IDC_BUTTON_HELP))
        {
...
extern HMODULE g_dllModuleHandle;
    ...
    VARIANTARG* pvarHelpContext)
{
    ...
    LOG(LOG_DEBUG) << L"pvarPrompt " << vbl::Hex((unsigned long)pvarPrompt) << L", pvarTitle " << vbl::Hex((unsigned long)pvarTitle)
                   << L", pvarDefaultValue " << vbl::Hex((unsigned long)pvarDefaultValue) << L", pvarX "
                   << vbl::Hex((unsigned long)pvarX) << L", pvarY " << vbl::Hex((unsigned long)pvarY) << L", pvarHelpFile "
                   << vbl::Hex((unsigned long)pvarHelpFile) << L", pvarHelpContext " << vbl::Hex((unsigned long)pvarHelpContext);
    ...
    INT_PTR ret = DialogBoxParamW(g_dllModuleHandle, MAKEINTRESOURCEW(ID_DIALOG_INPUTBOX), 0, InputBoxDlgProc, (LPARAM)&st);
    ...
    LOG(LOG_DEBUG) << L"DialogBoxParamW ret = " << vbl::Hex((unsigned long)ret) << L", GetLastError = "
                   << vbl::Hex((unsigned long)GetLastError()) << L", st.returnvalue "
                   << vbl::Hex((unsigned long)st.lpwstrReturnValue);
